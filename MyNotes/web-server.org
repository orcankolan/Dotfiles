...
*** create datbase and user, and set privilages
    #+BEGIN_SRC sql
      CREATE DATABASE odtutmdrupal DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci;
      GRANT ALL ON odtutmprupal.* TO 'odtuuser'@'localhost' IDENTIFIED BY '5tR0n6p@s5WorD';
      FLUSH PRIVILEGES;
      EXIT;
    #+END_SRC
...
** with other php modules:
   #+BEGIN_SRC sh
     sudo apt install php-cgi php-common php-fpm php-mysql php-gmp php-curl php-intl php-mbstring php-xmlrpc php-gd php-xml php-cli php-zip libssh2-php
   #+END_SRC
** php settings (php.ini):
   settings wiil be done in /etc/php/7.3/fpm/php.ini
   cgi.fix_pathinfo = 0
